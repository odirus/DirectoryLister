...
class DirectoryLister {
     ...
     * @access protected
     *
     * @notice $sortMethod 排序参数增加可选值 mod_timestamp_asc mod_timestamp_desc
     */
    protected function _arraySort($array, $sortMethod, $reverse = false) {
        ...
        // add start
        $newKeys = array();
        if (in_array($sortMethod, array('mod_timestamp_asc', 'mod_timestamp_desc'))) {
            $newArray = array();
            foreach($array as $fileName => $fileInfo) {
                $timestamp = $fileInfo['mod_time'];
                $newArray[$timestamp . '-----' . $fileName] = $fileInfo;
            }
            $newArrayKeys = array_keys($newArray);

            if ($sortMethod == 'mod_timestamp_asc') {
                sort($newArrayKeys);
            } else if ($sortMethod == 'mod_timestamp_desc') {
                rsort($newArrayKeys);
            }
